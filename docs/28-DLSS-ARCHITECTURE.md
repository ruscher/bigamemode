...
| `graphics/optiscaler.rs` | release records, API release-list parsing, download with `curl`, listing check and `bsdtar` extraction, ini reading and editing, payload, log reading (FSR 3.1 proven, FSR 4 never claimed) |
| `graphics/versions.rs` | which release a game gets — Recommended (tested) / Latest stable / Pinned — from a release list cached beside the downloads and refreshed at most daily; the update offer; the installed release found again by archive hash |
| `graphics/outcomes.rs` | local record of measured benchmark arms (game, GPU model, setup, runs); what they say about OptiScaler vs the game's own upscaler, with the benchmark tests |
| `graphics/gamedb.rs` | the short game list carried in the program (`bigame-core/data/graphics-games.toml`) plus the user's own; API default, prefer, block, tested version — never an anti-cheat unblock |
...
| `graphics/mod.rs` | facade: `analyze`, `install`, `update`, `go_back`, `update_offer`, `remove`, `repair`, `status`, `status_running`, `installed`, `launch_disables`, `target_for_process` |
...
| Release list | `~/.cache/bigame-mode/graphics/optiscaler/releases.json` | what GitHub lists as stable with a checksum; fetched again after a day, only from a page that needs it |
| Measurements | `~/.local/state/bigame-mode/graphics-outcomes.json` | local only; read by the planner, never sent |
| The user's game list | `~/.config/bigame-mode/graphics-games.toml` | overrides the list carried in the program |
...
- **Update = remove, then apply.** The original of every file is always the
  file from before BiGame-mode, never a previous BiGame-mode payload. Both
  releases are in the cache before the game changes; if the new one fails to
  apply, the previous one is applied again. The manifest keeps the previous
  version, and "Go back" is an update to it.
- **Offered, never forced.** A newer release is offered on the game's page —
  Update · Skip · Keep this version — and never applied during a launch.
- **What this machine measured beats what is known in general** — but only
  when the benchmark tests settle it, and never against native DLSS on RTX
  (the runs compared OptiScaler with the game's *other* upscaler).
- **The GPU a game renders on is observed, not assumed.** NVIDIA cards are
  discrete whatever sysfs says; a running game's render GPU is its open
  `/dev/nvidiaN`, then the secondary of several render nodes. DLSS is offered
  only on a model known to be RTX.
- **FSR 4 is never claimed from configuration or the log.** The log proves
  which backend was created, and proves FSR 3.1 when FSR 4 was off or AMD's
  runtime failed to load (the UI then says "FSR 3.1"); whether the FSR 4 model
  runs is logged only at debug level and shown by OptiScaler's overlay, so
  otherwise the UI says "FSR".
...
- **Local only.** Nothing is sent anywhere. The network is used for the
  user-initiated download from the project's GitHub release and for the
  release list (GitHub API, at most once a day, when a page that shows the
  offer is open).
