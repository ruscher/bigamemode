...
- Health messages (Diagnostics → System health) are English only: the core
  file that produces them is not in the translation list.
- Details pings the latency target once a second while the page is shown.
- The profile editor offers V-Cache modes on CPUs without 3D V-Cache (the
  wizard and Tuning already hide or explain them).
