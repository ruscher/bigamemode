...
//!
//! Usage: `launch_plan <program> [args…]` — prints the plan, then runs it with
//! the plan's environment and waits. Used to check on real hardware that a
//! plan does what it says (e.g. `launch_plan glxinfo -B` on a hybrid laptop).
fn main() -> anyhow::Result<()> {
    let args: Vec<String> = std::env::args().skip(1).collect();
    let (program, rest) = args
        .split_first()
        .ok_or_else(|| anyhow::anyhow!("usage: launch_plan <program> [args…]"))?;
