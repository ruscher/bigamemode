...
fn launch_context(target: &Target) -> plan::Context {
...
pub fn analyze(target: &Target, cfg: &config::AiGraphicsConfig) -> Analysis {
    ...
    let plan = plan::plan(&report, cfg, &launch_context(target));
...
pub fn install(target: &Target, plan: &plan::Plan) -> anyhow::Result<manifest::Manifest> {
    let o = plan
        .optiscaler
        .as_ref()
    ...
    tracing::info!(target: "graphics", game = %target.process, "graphics enhancement installed; active from the next start");
    Ok(m)
}
...
pub fn repair(target: &Target) -> anyhow::Result<Vec<PathBuf>> {
    ...
    let release = optiscaler::Release::recommended();
    let cached = optiscaler::fetch(&optiscaler::cache_dir(), &release)?;
